...
import { Programa, MetodoPago, Vendedora } from '@/lib/types'
...
interface VentaFormProps {
  /** 'vendedora' = /registro (atribuye la venta a la vendedora autenticada). 'sac' = /reinscripciones (origen='sac', sin vendedora). */
  origen: 'vendedora' | 'sac'
  titulo: string
  historialHref: string
}

export default function VentaForm({ origen, titulo, historialHref }: VentaFormProps) {
  ...
  const [resultado, setResultado] = useState<{ ok: boolean; mensaje: string; inscripcionDesactivada?: boolean } | null>(null)
  ...
  useEffect(() => {
    async function load() {
      ...
      const [{ data: progData }, { data: mpData }] = await Promise.all([
  ...
  // Solo aplica para el flujo de vendedora: filtra programas por la vendedora autenticada
  useEffect(() => {
    if (origen !== 'vendedora') return
  ...
  async function handleConfirm() {
    ...
    try {
      ...
      const inscribirData = await inscribirRes.json()
      const inscripcionDesactivada = Boolean(
        inscribirData?.results?.some((r: { inscripcion_automatica_desactivada?: boolean }) => r.inscripcion_automatica_desactivada)
      )

      setShowModal(false)
      setResultado({
        ok: true,
        mensaje: inscripcionDesactivada
          ? 'Venta registrada. Inscripción automática desactivada temporalmente — la venta quedó pendiente, se inscribirá cuando se active.'
          : 'Venta registrada e inscripción iniciada.',
        inscripcionDesactivada,
      })
      setTimeout(() => router.push(historialHref), inscripcionDesactivada ? 3500 : 2000)
    } catch (err: unknown) {
  ...
  const paisIdentificador = origen === 'vendedora'
    ? (vendedora?.pais ?? 'CL')
    : (seleccionados[0]?.programa.pais ?? 'CL')
  ...
  if (resultado) {
    return (
      <div className={`rounded-xl p-6 text-center ${resultado.ok ? (resultado.inscripcionDesactivada ? 'bg-amber-50 text-amber-800' : 'bg-green-50 text-green-800') : 'bg-red-50 text-red-800'}`}>
        <p className="text-lg font-medium">{resultado.ok ? (resultado.inscripcionDesactivada ? '⚠ Registrada' : '✓ Listo') : 'Error'}</p>
  ...
  return (
    <>
      <div className="max-w-2xl mx-auto">
        <h1 className="text-xl font-semibold text-gray-900 mb-6">{titulo}</h1>
